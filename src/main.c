     		...
     		switch(LMS_Ctrl_Packet_Rx->Header.Command)
     		{
 				...
 				case CMD_BRDSPI16_RD:
 					...
 					for(block = 0; block < LMS_Ctrl_Packet_Rx->Header.Data_blocks; block++)
 					{
 						...
 						CyU3PSpiTransmitWords (&LMS_Ctrl_Packet_Rx->Data_field[0 + (block * 2)], 1); //reg addr MSB with write bit
 						CyU3PSpiTransmitWords (&LMS_Ctrl_Packet_Rx->Data_field[1 + (block * 2)], 1); //reg addr LSB
 				...
 				case CMD_BRDCONF_RD: //read config data from board
					...
					case 0x8C://CMD_MEMORY_WR:
						...
						unsigned int target_id = (LMS_Ctrl_Packet_Rx->Data_field[10] << 8) | (LMS_Ctrl_Packet_Rx->Data_field[11]);

						switch (target_id) //TARGET
						{
							case 1: //fx3
								...
								switch (LMS_Ctrl_Packet_Rx->Data_field[0]) //PROG_MODE
								{
									...
									case 1: //PROG_MODE = 1 (destroy current firmware in flash)
										...
										if(current_portion == 0)//beginning
										{
											...
											if( FX3_FlashSpiEraseSector(CyTrue, 0) != CY_U3P_SUCCESS) cmd_errors++;
...
void FPGA_config_thread(uint32_t input)
{
	...
	for (fpga_byte = 0; fpga_byte < fpga_bitstream_size; fpga_byte++)
	{
		...
		{
			Reconfigure_SPI_for_Flash ();
			FlashSpiTransfer((FLASH_LAYOUT_FPGA_BITSTREAM * FLASH_SECTOR_SIZE/FLASH_PAGE_SIZE) + flash_page, FLASH_PAGE_SIZE, flash_page_data, CyTrue);
